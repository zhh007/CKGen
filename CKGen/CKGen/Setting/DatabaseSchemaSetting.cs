        ...
        private static void forUpdate1(string filePath, IDatabaseInfo db)
        {
            ...
            foreach (ITableInfo tb in db.Tables)
            {
                ...
                if (xmlTable != null)
                {
                    ...
                    List<XElement> allXmlColumn = new List<XElement>();
                    foreach (IColumnInfo col in tb.Columns)
                    {
                        ...
                        allXmlColumn.Add(colEl);

                        if (!string.IsNullOrEmpty(local_desc))
                        {
                            col.Attributes["local_desc"] = local_desc;
                            colEl.Attribute("desc").Value = local_desc;
                        }
                    }
                    //delete column
                    var allColumn = xmlTable.Elements().ToList();
                    for (int i = allColumn.Count - 1; i >=0 ; i--)
                    {
                        if (!allXmlColumn.Contains(allColumn[i]))
                        {
                            allColumn[i].Remove();
                        }
                    }
                ...
                {//new table
                    XElement newXmlTable = createXmlTable(tb);
                    allXmlTable.Add(newXmlTable);
                    root.Add(newXmlTable);
                }
            }

            //delete table
            var allTable = root.Elements().ToList();
            for (int i = allTable.Count - 1; i >= 0; i--)
            {
                if (!allXmlTable.Contains(allTable[i]))
                {
                    allTable[i].Remove();
                }
            }
